...
export function registerPageRoutes(app: FastifyInstance, options: PageRouteOptions): void {
  ...
  app.get('/article/:id',async(request,reply)=>{const context=await options.auth.authenticate(request,reply);if(!context)return reply.redirect('/login');const raw=(request.params as {id?:unknown}).id,id=typeof raw==='string'&&/^\d+$/.test(raw)?Number(raw):0;if(!id||!options.articles)return reply.status(404).send('文章不存在');const row=await options.articles.getArticle(id);if(!row)return reply.status(404).send('文章不存在');return reply.type('text/html; charset=utf-8').send(views.article.split('__ARTICLE_TITLE__').join(escapeHtml(row.title)).replace('__ARTICLE_CONTENT__',sanitizeArticle(row.contentHtml)).replace('__ARTICLE_META__',`${escapeHtml(String(row.createdAt))} · ${escapeHtml(row.authorName)}`));});
}
